...
import { InvoiceRefundCreate } from "./InvoiceRefundCreate";
export const InvoiceRefund = () => {
  //Phiếu đề thu
  const dispatch = useDispatch();
  const Creater = localStorage.getItem("CreateBy");
  const CreateName = localStorage.getItem("CreateName");
  const [showAddInvoice, setShowAddInvoice] = useState();
  ...
  const Trl_spInvoice_Excel = () => {
    ConfirmAlert("Xác nhận", "Bạn có chắc muốn xuất?", () => {
      let dataexcel = state.data.map((item, index) => {
        return {
          "Hóa đơn": item.InvoiceType === "D" ? "Hoàn" : "Chi",
  ...
  const [titleShow, setTitleShow] = useState("TẤT CẢ PHIẾU HOÀN");
      ...
      <div className="content-wrapper">
                ...
                >
                  <div class="input-group" style={{ width: "100%" }}>
                    <input
                      type="text"
                      maxLength="500"
                      placeholder="Mã phiếu hoàn,Mã KH..."
                      class="form-control"
                      value={InvoiceSearch}
                ...
                <div className="row">
                  ...
                  <div class="col-sm-12 mb-4">
                    <div className="row">
                      ...
                      <div className="col-sm-12 mt-1">
                        Mã tour đăng ký: {invoiceView?.BookingCode || ""}
                      </div>
                      ...
                      <div className="clearfix"></div>
                      <div className="col-sm-12 mt-1">
                        Còn lại: {FormatMoney(invoiceView?.TotalRemain)} (Bằng
                        chữ: {MoneyToText(invoiceView?.TotalRemain)})
                      </div>
                      <div className="col-sm-6 mt-1">
        ...
        >
          <Modal.Header className="py-2 border-bottom-0 ">
            <Modal.Title className="ml-auto">PHIẾU HOÀN</Modal.Title>
            <button
              type="button"
              class="close"
              aria-label="Close"
          ...
          <Modal.Body class="p-1">
            <InvoiceRefundCreate
              ...
              handleShow={() => setShowAddInvoice(true)}
