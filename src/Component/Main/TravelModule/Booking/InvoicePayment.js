...
import { Alerterror, ConvertFileName, FormatMoney } from "../../../../Utils";
...
export const InvoicePayment = ({
  ...
  const [typePm, setTypePm] = useState("v0");
  ...
  const Trl_spInvoice_Save = async (isConfirm = 0) => {
    ...
    try {
      let pr = {
        ...
        Money: invoice.TotalMoney || 0, // ?
        VAT: vat,
        MoneyVAT: (vat * invoice.TotalMoney) / 100 || 0,
        MoneyPayment:
          invoice.TotalMoney - (vat * invoice.TotalMoney) / 100 || 0,
      ...
      if (respone.Status === "OK") {
        ...
        onInvoiceInfor({
          ...
          TypePm: typePm,
          RetailVisaId: DataObject?.RetailVisaId || 0,
          IsConfirm: isConfirm === 0 ? false : true,
        });

        Trl_spInvoice_List(
          invoice.InvoiceId === 0 ? respone.InvoiceId : invoice.InvoiceId
        );
        setFlagResetFile(1);
        setFlagResetFile(0);
      } else {
  ...
  const NewInvoice = () => {
    if (DataObject?.InvoiceId4 === 0) {
      setTypePm("V4");
    } else if (DataObject?.InvoiceId5 === 0) {
      setTypePm("V5");
    } else if (DataObject?.InvoiceId6 === 0) {
      setTypePm("V6");
    } else {
      Alertwarning("Không thể tạo phiếu thu thêm");
      return;
    }
    setInvoice({
      typeOfMoney: { value: 1, label: "VND" },
  ...
  useEffect(() => {
    ...
    } else if (DataObject?.InvoiceId4 && DataObject.TypePm === "V4") {
      Trl_spInvoice_List(DataObject.InvoiceId4);
    } else if (DataObject?.InvoiceId5 && DataObject.TypePm === "V5") {
      Trl_spInvoice_List(DataObject.InvoiceId5);
    } else if (DataObject?.InvoiceId6 && DataObject.TypePm === "V6") {
      Trl_spInvoice_List(DataObject.InvoiceId6);
    } else if (DataObject?.Payment1 && DataObject.TypePm === "VS1") {
      Trl_spInvoice_List(DataObject.Payment1);
    } else if (DataObject?.Payment2 && DataObject.TypePm === "VS2") {
      Trl_spInvoice_List(DataObject.Payment2);
    } else {
      setInvoice({
        typeOfMoney: { value: 1, label: "VND" },
        typeOfPayment: { value: 2, label: "Chuyển khoản" },
      ...
      try {
        ...
        if (respone.Status === "OK") {
          ...
          await onInvoiceInfor({
            ...
            BookingDetailId: DataObject.BookingDetailId,
            CustomerId: DataObject.CustomerId,
            TypePm: typePm, // V1 là invoice1, V2 là invoice2...
      ...
      <div class="card m-1 card-shadow shadow">
        <div class="card-header card-header-custom p-0 px-1 mt-1 d-flex align-items-center ">
          <div class="row d-flex align-items-center justify-content-center w-100">
            ...
            <div class="col-sm-12 col-md-6">
              {(invoice?.StatusId === 2 ||
                (invoice.Creater !== Creater && invoice.InvoiceId !== 0)) && (
                <div>
                  <button
                    type="button"
                    class="btn btn-sm btn-warning pull-right margin-left-5"
                    onClick={() => Trl_spInvoice_Save(3)}
                  >
                    Cập nhật
                  </button>
                  <button
              ...
              <div class="form-group">
                ...
                <div class=" input-group">
                  <input
                    ...
                    class="form-control2"
                    value={
                      DataObject?.CustomerCode ? DataObject?.CustomerCode : ""
                    }
                    readOnly="true"
                  />
                </div>
