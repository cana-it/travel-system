...
const BookingPrintComp = ({
    ...
    return (
        <>
            <div className="row booking landscape" style={{ backgroundColor: "#fff" }}>
                <div class="col-sm-2 text-center">
                    <Img src="../dist/img/logo.png" height="70px" style={{ marginleft: '20%', marginTop: "20px" }} />
                </div>
                <div class="col-sm-8" style={{ textAlign: "center", verticalAlign: "middle", lineHeight: "50px", fontSize: "25px", textTransform: 'uppercase', paddingTop: "20px" }}>
                    {Invoice?.BookingName}
                </div>
                ...
                <div class="col-sm-12 col-md-12 mt-3">
                    <table className='table bg-white table-work-break table-bordered' cellPadding={1} width={"100%"}>
                        <thead style={{}}>
                            <tr>
                                <th>No</th>
                                <th>Full Name</th>
                        ...
                        <tbody>
                                ...
                                Invoice?.dataDetail?.filter(p => p.StatusCustomer === 'OK').map((item, index) => {
                                    return (
                                        <tr key={"PrinDetai" + index}>
                                            ...
                                            <td>{FormatMoney(item?.Deposit1 || 0)}</td>
                                            <td>{FormatMoney(item?.Deposit2 || 0)}</td>
                                            <td>{FormatMoney(item?.Deposit3 || 0)}</td>
                                            <td>{item?.Sale || ''}</td>
                                            <td>{item?.Note || ''}</td>
                                        </tr>
                                    )
                            ...
                            {
                                 Invoice?.dataDetail?.filter(p => p.StatusCustomer === 'OK').length === 0 && <tr><td colSpan={23} class="text-center">Chưa có thông tin </td></tr>
                            }
                        </tbody>
                    </table>
...
export const BookingPrint = React.memo(BookingPrintComp)