...
export const InvoiceDefault = ({
  ...
  const Trl_spInvoice_Save = async () => {
    ...
    try {
      let pr = {
        ...
        MoneyPayment:
          invoice.TotalMoney - (invoice.isVAT ? 10 : 0 * invoice.Money) / 100,
  ...
  useEffect(() => {
    ...
    } else if (invoiceStyle.TypeInvoice === 2 && Booking?.LandTourInvoiceId) {
      Trl_spInvoice_List(Booking.LandTourInvoiceId);
    } else if (invoiceStyle.TypeInvoice === 3 && idPlane?.InvoiceId) {
      Trl_spInvoice_List(idPlane.InvoiceId);
    } else if (invoiceStyle.TypeInvoice === 4 && idTourLeader[0]?.InvoiceId) {
      Trl_spInvoice_List(idTourLeader[0]?.InvoiceId);
    } else {
      setInvoice({
