...
import { OtherServicePrint } from "./OtherServicePrint";
export const CreateServiceOther = React.forwardRef((props, ref) => {
  let prm = useLocation();
  useEffect(() => {
  ...
  const per = [15, 18, 40, 61, 24, 1];
  const initialState = !per?.includes(+CreateBy)
    ? { hiddenColumns: ["CostServiceOther", "ProfitableServiceOther"] }
    : {};
      ...
      const pr = bookingDetails
        ...
        .map((item) => {
          ...
          return {
            ...
            DetailServiceOther: item.DetailServiceOther,
            CostServiceOther: item.CostServiceOther,
            ProfitableServiceOther: item.ProfitableServiceOther,
          };
        });
      if (check === 1) {
  ...
  const columnsDetail = useMemo(
    () => [
      ...
      {
        Header: "Lãi ròng",
        accessor: "ProfitableServiceOther",
        disableResizing: true,
        width: 110,
                ...
                <div className={`justify-content-end mt-2 d-flex `}>
                  <button
                    className={`btn btn-info mr-2 ${
                      dataTableError.some((item) => item.Checked)
                        ? ""
                        : "d-none"
                    }`}
