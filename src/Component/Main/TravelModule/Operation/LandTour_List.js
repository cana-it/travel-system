...
import { LandTourForm } from '../Booking/LandTour'

export const LandTour_List = () => {
    ...
    const Trl_spBookingLandTour_List = async () => {
        ...
        try {
            ...
            const params = {
                ...
                func: "Trl_spBookingLandTour_List"
            }
            ...
            if (list.length > 0) {
                ...
                list.reduce(function (res, value) {
                    ...
                    res[value.BookingId].qty += value.BookingTourLandTourId;
                    return res;
                }, {});
    ...
    const Trl_spBookingLandTour_Excel = () => {
    ...
    const Trl_spBookingLandTour_View = async (row) => {
        
        await setdataPlane(row._original);
        openModal();
    }

    const Trl_spBookingLandTour_Print = async (row) => {
    ...
    const columns = [
        {
            ...
            Cell: ({ row }) => (
                <div style={{ textAlign: 'center' }}>
                    <button className="btn btn-sm btn-success" onClick={e => Trl_spBookingLandTour_View(row)}> <i class="fa fa-edit"></i>Cập nhật</button>
                </div>
            ),
            width: 200,
            fixed: "left"
    ...
    const Trl_spBookingLandTour_Approve = async () => {
        ...
        try {
            ...
            if (window.confirm("Bạn có chắc chắn muốn xác nhận?")) {
                ...
                const params = {
                    Json: JSON.stringify(pr),
                    func: "Trl_spBookingLandTour_Approve"
                }
            ...
            <div class="card-header border-left-red PrintInvoiceHide">
                <div class="row">
                    <div class="col-sm-12 col-md-4" >
                        <h3 class="card-title font-weight-bold">Quản lý LandTourr({data.length})</h3>
                    </div>
                    <div class="col-sm-12 col-md-8 margin-top-5s">
                        <button disabled={data.length > 0 ? false : true} onClick={Trl_spBookingLandTour_Excel} type="button" class="btn btn-sm btn-success pull-right margin-left-5">
                            <i class="fa fa-download pr-2"></i>
                            Excel
                        </button>
                        <button onClick={Trl_spBookingLandTour_List} type="button" class="btn btn-sm btn-danger pull-right margin-left-5">
                            <i class="fa fa-search pr-2"></i>
                            Tìm kiếm
                        </button>
                        <div class="form-group pull-right" style={{ marginTop: '-4px' }}>
                            <div class="input-group" style={{ width: '100%' }}>
                                <input type="text" maxLength="500" placeholder="Status Plane,Booking..." class="form-control" value={LandTourSearch} onChange={e => setLandTourSearch(e.target.value)} />
                            </div>
                        </div>
                    ...
                    <div className="col-12 col-sm-6 col-md-3">
                        <div className="info-box">
                            ...
                            <div className="info-box-content">
                                <span className="info-box-text">LandTour</span>
                                <span className="info-box-number float-left">
                                    {data.filter(p => p.BookingTourLandTourId !== undefined).length}
                                </span>
                            </div>
                        </div>
                    </div>
